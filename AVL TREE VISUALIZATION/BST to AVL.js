...
function BSTAVL() {
	...
	this.balance = function(cur) {
		if (cur.balancefactor() == 2) ///left
		{
			if (cur.left.balancefactor() == -1) ///if this condition is valid so it is left right
				cur.left = this.RotateLeft(cur.left); ///here i convert it to left left rotation

			/// When performing this operation, I can confidently identify that I am dealing with a left-left case, and consequently, I transform it into a balanced tree.
		...
		} else if (cur.balancefactor() == -2) ///right
		{
			...
			if (cur.right.balancefactor() == 1) ///if this condition is valid so it is right left
				cur.right = this.RotateRight(cur.right); /// here converting it to right right

			///Upon executing this action, I am certain that I have entered a right-right case, and accordingly, I transform it into a balanced tree.
	...
	this.pre = function(cur = this.root) {
		var numbers = [];
		if (cur != null) {
			numbers = [cur.value];
			numbers = numbers.concat(this.pre(cur.left));
			numbers = numbers.concat(this.pre(cur.right));
		}
		return numbers;
	}

	this.post = function(cur = this.root) {
		var numbers = [];
		if (cur != null) {
			numbers = numbers.concat(this.post(cur.left));
			numbers = numbers.concat(this.post(cur.right));
			numbers.push(cur.value);
		}
		return numbers;
	}
